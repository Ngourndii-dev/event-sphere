      ...
      <Link href="/" className="flex items-center space-x-2">
        ...
        <div className="hidden md:flex items-center space-x-6">
          {navItems.map((item) => (
            ...
            >
              <span className={`transition-colors duration-200 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} group-hover:text-green-500`}>
                {item.icon}
              </span>
              <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-700'} group-hover:text-green-500`}>
                {item.name}
              </span>
              <span
      ...
      >
        ...
        >
          {navItems.map((item, index) => (
            ...
            >
              <span className={`${isDarkMode ? 'text-green-400' : 'text-green-600'}`}>{item.icon}</span>
              <span className={`text-base font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}>{item.name}</span>
            </Link>
          ))}
        </div>
      </div>
...
export default function News() {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `linear-gradient(to bottom, ${isDarkMode ? 'rgba(17, 24, 39, 0.7)' : 'rgba(249, 250, 251, 0.7)'}), url(${isDarkMode ? noir.src : blanc.src})`,
    ...
    >
      ...
      <Separator />
      <main className={`max-w-5xl mx-auto px-4 sm:px-6 py-12 pt-20 transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
        <header className="flex items-center justify-between mb-10">
          <div className="flex items-center space-x-2 animate-float">
            <Globe className={`w-7 h-7 ${isDarkMode ? 'text-green-400' : 'text-green-600'} transition-colors duration-300`} aria-hidden="true" />
            <h1
              className={`text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r 
                ${isDarkMode ? 'from-green-400 to-teal-300' : 'from-green-600 to-teal-500'}`}
            >
              Event News
            </h1>
          </div>
